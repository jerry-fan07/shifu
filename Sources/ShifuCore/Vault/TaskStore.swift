...
public enum TaskStore {
    ...
    /// were later re-grouped away. Deleting is safe: the key re-mints the
    /// moment it earns real time, and the activities keep their ledger time,
    /// just task-less. Day logs and work notes recompile without them.
    @discardableResult
