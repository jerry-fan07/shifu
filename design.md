...
- **Tasks**: the analyzer groups activities into ongoing tasks. With an LLM backend, `SemanticTaskGrouper` assigns blocks to *intent-level* tasks — "Applying to YC afterparties", "Booking flights for the trip" — spanning apps and domains: each block's evidence plus a roster of recent semantic tasks goes to the model, which joins existing tasks or mints new ones (title + one-line gist), confidence-gated (0.6) and attempt-capped (3, like §4.2's classifier). The prompt is built around what a human watching over your shoulder actually uses (`SemanticTaskEvidence.swift`). **The roster is a weighted prior, not a list of names**: every entry carries its minutes and days logged over the 14-day window, days since last worked, and the domains its time actually went to — so a `partiful.com` block matches the task that already lives there, not merely the one whose title reads closest. **Work is sticky**: the window's already-grouped time rides along read-only (id-less, so it can't be re-assigned) and *interleaved chronologically among the candidates*, with the instruction to prefer the surrounding task when a block's own evidence is thin — the strongest cue a human has, since switches are punctuated events. Consecutive blocks of one task collapse into a single span line ("~ Thu 09:12-11:40 148m ×12 → t37"), so a whole afternoon costs one line rather than forty, and only spans within half an hour of the batch's own hours render — a batch that shrank to fit its budget takes its context down with it. *(Rewritten 2026-08-01. It was a `LIMIT 6` lookup anchored ±6 h on the batch's **first** block, which was only ever right for a short batch: `candidateLimit` was an item count rather than a token budget, so it cut in 52% of the dogfood window's hourly runs and batches spanned a median 33.8 h. 67% of candidates were being shown what happened around a different day, and the prompt's "consecutive blocks usually continue the same task" was false of the list the model was reading. Measured over all 302 hourly windows of the dogfood history: candidates with assigned context inside 15 minutes went from 15% to 65% — the ceiling is the 35% sitting in stretches where nothing was ever grouped, which no prompt can conjure.)* Batches are now cut by `LLMTokens.estimate` alone (invariant 7), against prompt **and** answer: `answerTokens(candidates:)` scales the response reserve with the batch and is what `complete(maxTokens:)` asks for, where a flat 2 000 used to be — measured worst case is 4 118, so batches above ~140 candidates were silently outgrowing their own answer, and the fast slot has no escalation retry. Worst rendered prompt + answer over the same 302 windows is 32 253 of the 60 000-token window. That prior is explicitly **fenced against interruptions**: a short messaging/social/entertainment detour between two stretches of one task is a break from it, never part of it — the *return* is the continuity evidence, not the detour. **The cheapest claims never reach the model**: an unplaced block with the same task running on both sides of it, every gap inside one sitting (`Sessionizer.gapThresholdMs`), inherits the sandwich's task outright (`SemanticTaskInheritance.swift`) — zero tokens, run before the model pass so its candidate pool is already smaller; break categories (messaging/social/entertainment) never inherit, the same interruption fence, and exhausted `sem_attempts` are honoured, so inheritance can't back-door a verdict the model refused. Two blocks qualify: a sub-`minBlockMs` glance, which will never be worth a token of its own, and — *added 2026-08-01* — a **longer block that happened in the same app and domain as one of the slices**, which says what duration alone cannot: same place, same sitting, same task either side. The scope was picked by replaying the rule over the dogfood window and scoring it against the model's own 2 266 labels — the counterfactual "would this have reached the same answer without a call?". Sub-minute alone covers 36.0% of labelled blocks at 94.6% agreement; the source test adds 125 blocks at 96.8%, taking the rule to 41.5% at 94.9% — more coverage *and* slightly better precision. The looser rules were measured and rejected: nearest-assigned-block-in-time reaches 99.6% coverage at only 72.3%, and dropping the source test to admit every long block gives 51.1% at 90.8%, which is one wrong task in eleven. **Text similarity is not the tool here** — a third measurement (after NMF was rejected twice, §12) found `NLEmbedding` over the 52 task names actively misleading: the true duplicate pair scores 0.727 while unrelated "Checking terminal login status" ↔ "Checking western blot automation status" scores 0.842. The label is *intent*, and the same site serves a dozen intents; temporal and structural adjacency is where the free signal is. **The evidence per block is denser at the same token cost**: observations are sampled across the block's whole span (first, last, and an even fan between) instead of its opening minutes, and browser blocks carry sanitized page identities (`github.com/org/repo` — origin plus two path segments, query and fragment dropped, then re-redacted, since `observations.url` is stored raw) instead of the bare host. Sub-minute glances are pooled, not discarded: one 40-second glance is not worth tokens (`minBlockMs`), but the same app and domain glanced at a dozen times in a quarter hour is one act of attention — a hop-style tool (Conductor in the dogfood ledger) produces *only* such slivers — so they coalesce into **runs** (`SemanticTaskSlivers.swift`): same app+domain, gaps to 15 min, other apps freely in between, pooled active time clearing the same 60 s floor one block must (a 48 h dogfood window collapsed ~1.5k sliver candidates into ~105 runs; the lone glances left behind are what the floor exists for). A run is one candidate at one block's token cost — its evidence fans across the members through the same five-sample spread, its prompt line reads as what it is ("7 visits · 3m over 41m"), it is assigned all-or-nothing (hops straddling two intents land on one key, bought back by the 15-minute cut), and a declined run burns an attempt on every member, so exhausted ones drop out and the run re-forms smaller, no stored identity. The candidate limit is a **quota** — long blocks take slots first, runs fill the remainder by pooled time; admitting by recency alone once let a hop-heavy afternoon buy 59 glances and 1 block of real work. It is now a backstop (300) rather than the binding constraint it was at 60, and runs return to time order once the quota has chosen them, so the two quotas no longer interleave into a sample that jumps around the window. Everything stays token-budgeted (CLAUDE.md invariant 7): backends under 16k context get the compact tier — the twelve heaviest tasks, names and gists only, and at most three context lines — which keeps on-device Foundation Models' 4k window in budget. The verdict lands in `activities.sem_key`, carried across rebuilds by span identity. **An unreadable answer is not a verdict**: `parse` returns nil when no *balanced* JSON object can be decoded — truncation, a refusal, prose with a stray brace — and the batch then keeps its `sem_attempts` and retries next hour, where a decoded-but-empty verdict still burns one. The distinction is what stops a batch that outgrew its answer reserve from quietly spending three attempts on blocks no model ever judged; it matters more now that one batch can carry a whole window. Blocks the model can't place — and all blocks when no backend is configured — fall back to the mechanical key: classified topic, else domain, else app (`TaskGrouper`) — an `app:` task minting under the installed app's own Launch Services name rather than the bundle-id tail, which is wrong exactly when the tail is generic (`com.conductor.app` would otherwise mint a task literally named "app"). With a backend configured the fallback is also a **last resort** (`TaskGrouper.MechanicalMinting`): a *container* key — `domain:`/`app:`, the namespaces that name where time went rather than what it was for — mints a task, or keeps attaching to an existing one, only from blocks the semantic pass has *finished declining*, summing past `minNewTaskMs`; `TaskStore.prune` reaps existing container tasks below the same bar on sight, on the same flag, so reap and mint cannot disagree. "Instagram" stays a task because the model keeps declining to place it; a 40-second message check stays task-less instead of becoming a task called "MobileSMS". A rename or a merge into the task spares it from both (a merge rewrites the absorbed blocks' `sem_key` to the survivor's key, so the fold holds across rebuilds), and the policy derives from *configuration*, never from whether a pass's calls succeeded — a network timeout must not mint; with no backend at all, minting is unrestricted and the rules-only roster stands whole (§10). The fallback is hardened against fragmentation: the classifier prompt lists recent topics and the model repeats one verbatim when a block continues that effort (keys only recur if wording recurs), a never-seen mechanical key mints a task only once a window shows ≥ 5 min behind it (`minNewTaskMs`), and sub-threshold, never-renamed, never hand-filed tasks inactive for a week are pruned (`TaskStore.prune`) — passing subjects stay task-less while their time still counts in the ledger. System shell surfaces — the lock screen, the Dock, one-shot dialogs, Shifu's own UI, bundle-less `unknown.<pid>` processes — never enter the ledger at all (`TaskGrouper.isSystemBundle`, applied where `LedgerBuilder` writes blocks; `v26-system-shell-purge` removed the rows written before that gate): they carry no topic or domain, so they'd bottom out at the `app:` key and mint permanent nonsense tasks ("loginwindow") that accrue time daily and never go stale — and keeping them out of `activities` keeps them out of grouping, the Time page (§7), and every query added later, by construction. Browser-internal pages get the same treatment one level down: a block's domain derives from http(s) URLs only (`Sessionizer.domain`), so `chrome://`/`chrome-extension://`/`devtools://` surfaces ("new-tab-page", "contextual-tasks", "omnibox-popup.top-chrome", extension ids) never mint `domain:` tasks — the dogfood ledger had 145 min filed under "contextual-tasks" — and prune reaps pre-gate ones on sight (`TaskGrouper.isBrowserInternalHost`), a rename again being the only shield. Tasks span days (the key recurs), are renameable, and renames survive re-analysis (keys never overwrite names, and the semantic pass never overwrites either).
...
2. ~~Local model choice: Apple Foundation Models framework (zero bundle cost, OS-version-gated) vs. bundled MLX model (~2 GB, works everywhere) — ship both with runtime selection?~~ Resolved 2026-07: neither. Foundation Models shipped first and lost on its 4k window, weak labels, and macOS 26+ gate; both on-device paths were dropped for DeepSeek (`deepseek-v4-flash` default) as the sole backend (§4.2). *Reopened 2026-07-31 with different candidates and a measured go (§12): Qwen3.5-9B at a 16k window matches DeepSeek on cards (89%) and groups plausibly on Apple Silicon ≥16 GB; the 4B serves 8 GB Macs under the 24-block grouping cap. Model choice by `hw.memsize`; Intel stays rules-only/BYO-key. Final ship gate is the user's blind week on task-title quality.* *Built 2026-08-01 as the qwen edition's auto-setup (§4.2): onboarding downloads the tier's GGUF, the bundle carries a from-source llama-server, the analyzer owns its lifecycle. llama.cpp stayed the engine — the MLX rewrite from the reopening note was not needed for the release shape and is not planned.*
