...
Shifu is a full desktop app with a menu bar companion, and it is set somewhere: a mountain with a temple on it, which is the metaphor the whole product already runs on — a path, a climb, a guide who has been up it. The app is that place, not a dashboard with a mascot pasted on.

**The artwork** is flat 2D vector, drawn entirely in `Canvas` — no bitmaps, no image assets, so every surface is resolution-free and themable from a palette.

- `ShifuApp/Landscape.swift` — the scene. A sky wash, angular far peaks, a **pagoda in the fog** on the mid range's summit, and two rolling near dunes, with atmospheric perspective doing all the depth work: each layer forward is darker and less washed toward the sky. Four palettes (`SkyTime`: dawn / day / dusk / night) follow the wall clock, so opening the app at 11pm gives a moonlit window, not the same white card. Each palette also publishes the ink color for text laid over its *upper* sky — the one band whose contrast is guaranteed, and the only place the app puts words on the painting.
...
- **Main window**: a trail on the left and one place beside it. Sidebar rows are stations on a dashed route — Shifu's mark up top, *Today*, then **The path** (*Time*, *Themes*, *Task log* — §5.3), **The mind** (*Practice* — §5.2 — and *Scrolls*, vault search — vault-features.md §4), **The watch** (*Radar*) — with a status footer carrying capture state, pause, Work Mode, and the door to Settings, and the mountain itself holding up the bottom edge. Charts native SwiftUI; no web views.
- Every page except Today wears one `PageScaffold`: region eyebrow, title, one line on what the page is for, and the page's own controls in the header's trailing slot. Nothing goes in the window toolbar — one bar of chrome per page, and it is that one.
- **Today**: the scene, full width, with the hour's sky behind a time-of-day salutation and the day's aphorism; the tracked-today total on the surface strip beneath it; the counts that want attention; the day's stones; today's task log.
