...
- **The Practice page** (§7): **suggested decks** (each with its title, source task, and sample cards, and Create/Dismiss), review-activity calendar heatmap (from `srs_reviews`), per-card urgency overview (overdue / due today / new / soon / scheduled), and the deck picker. Decks still building show a progress row. The review session is the one screen pushed from here — there is no inbox.
