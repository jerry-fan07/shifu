...
- **Sub-minute hop admission for `CardBuilder` (§5.3)** — the semantic
  grouper now admits an app's sub-minute slivers by accumulated weight
  (`pendingSamples`); `CardBuilder` still holds the plain 60 s floor. Likely
  right as is — a sliver's raw evidence is smaller than a card would be —
  and a slow drip of hops that never clears 5 min inside one window still
  waits; revisit either only if `llm_usage` or the dogfood ledger says the
  residue is real.
