...
Shifu is a full desktop app with a menu bar companion, dressed as what it is: a small dojo. The **Dojo design system** (`ShifuApp/Dojo.swift`) sets the whole look — warm paper surfaces, ink text, and one terracotta accent, the sensei's robe. Type stays the system sans; a serif italic is reserved for the mentor's own voice (the `Wisdom` aphorisms — original lines, used only in the Today greeting and empty states). The **miniature sensei** himself (`SenseiFigure`, a vector drawing: straw kasa, terracotta gi, hands folded into his sleeves) appears in the sidebar mark, on the Today page, and in every empty state, so a blank screen still feels inhabited — he rests when capture is paused, and beams when a review session ends. Minimalism still governs (§1, principle 2): generous whitespace, no gamification, badges only where a number is actionable (cards due, radar suggestions), no settings page longer than one screen.

- **Menu bar item** (the always-visible surface): martial-arts glyph while watching, zzz while paused; Work Mode toggle, "Review · N due", "Today: 4.2 h work · 1.1 h learning", Pause 1h / until tomorrow, Open Shifu, Settings, Quit.
- **Main window**: a sidebar of places and one page beside it. The sidebar groups them the way the mentor thinks — *Today*, then **Path** (*Time*, *Themes*, *Task log* — §5.3), **Mind** (*Practice* — §5.2 — and *Scrolls*, vault search — vault-features.md §4), **Watch** (*Radar*) — with a status footer carrying capture state, pause, Work Mode, and the door to Settings. Charts native SwiftUI; no web views.
- **Today**: the greeting page — time-of-day salutation, the day's aphorism, a tracked-today hero with its top categories, the counts that want attention, and today's task log.
- **The Time page** carries one window (Day / Week) and one lens (Category / Theme / Task, §5.3) across two modes, all three controls in the window toolbar:
...
- **Onboarding**: a 4-screen flow in the sensei's voice that (1) explains exactly what is and isn't captured, (2) requests Screen Recording + Accessibility permissions, (3) sets exclusions (password managers, banking category, private browsing), (4) picks analysis backend (local-only default).
